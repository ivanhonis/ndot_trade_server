...
class NTrader:
	...
	def update_ai(self):
		...
		def get_project_update_str(project):
			tf_name = self.tfm_mp.slots_dir + f"/{project}/nDot_TF_MODEL_{project}.h5"
			minmax_name = self.tfm_mp.slots_dir + f"/{project}/nDot_MinMaxScaler_{project}.pickle"
			config_name = self.tfm_mp.slots_dir + f"/{project}/nDot_PRO_{project}.txt"
			if os.path.exists(tf_name) and os.path.exists(minmax_name) and os.path.exists(config_name):
				dtf = os.path.getmtime(tf_name)
				dminmax = os.path.getmtime(minmax_name)
				dconfig = os.path.getmtime(config_name)
				udstr = str(dtf) + str(dminmax) + str(dconfig)
			else:
				udstr = ""
			return udstr
		
		def get_project_fnames(project):
			tf_name = self.tfm_mp.slots_dir + f"/{project}/nDot_TF_MODEL_{project}.h5"
			minmax_name = self.tfm_mp.slots_dir + f"/{project}/nDot_MinMaxScaler_{project}.pickle"
			config_name = self.tfm_mp.slots_dir + f"/{project}/nDot_PRO_{project}.txt"
			return tf_name, minmax_name, config_name
	...
	def get_socket_name_kline_1m(self, symbol):
